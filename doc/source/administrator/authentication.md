...
Make sure that the [oauth\_callback\_url]{.title-ref} matches the one
you set in GitHub.
...
For more information on authenticating with Google oauth, see the
`google_oauth`{.interpreted-text role="ref"}.
...
setting [login\_service]{.title-ref} you can customize the label on the
login button.
...
Full Example of Google OAuth2 {#google_oauth}
-----------------------------
...
7.  Set \"Authorized redirect URIs\" to be your hub\'s URL followed by
    \"/hub/oauth\_callback\". For example,
    [https://your-jupyterhub-domain/hub/oauth\_callback]{.title-ref}.
...
[jupyterhub\_config.py](https://github.com/jupyterhub/zero-to-jupyterhub-k8s/blob/master/jupyterhub/files/hub/jupyterhub_config.py#L353).

### Example LDAP Configuration

[server\_address]{.title-ref} and [bind\_dn\_template]{.title-ref} are
